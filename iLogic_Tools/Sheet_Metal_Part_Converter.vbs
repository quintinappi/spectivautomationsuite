...
Sub ConvertPartToSheetMetal(partDoc, thickness)
    ...
    WScript.Echo ""
    WScript.Echo "Saving document to ensure SelectSet works..."
    
    On Error Resume Next
    partDoc.Save
    
    If Err.Number <> 0 Then
        WScript.Echo "Warning: Could not save - " & Err.Description
        Err.Clear
    Else
    ...
    ' Give time for selection to clear
    WScript.Sleep 200
    ...
    WScript.Echo ""
    WScript.Echo "=========================================="
    WScript.Echo "*** LOOK AT INVENTOR NOW ***"
    WScript.Echo "The large face should be highlighted GREEN"
    WScript.Echo "=========================================="
    WScript.Sleep 2000
    ...
    ' Execute the command
    WScript.Echo ""
    WScript.Echo "Starting Convert to Sheet Metal command..."
    convertCmd.Execute
    ...
    ' Step 5: User interaction required
    WScript.Echo ""
    WScript.Echo "=========================================="
    WScript.Echo "*** ACTION REQUIRED ***"
    WScript.Echo "1. The Convert dialog is now open"
    WScript.Echo "2. CLICK ONCE on the GREEN highlighted face"
    WScript.Echo "3. Then press ENTER here to continue..."
    WScript.Echo "=========================================="
    WScript.Echo ""
    ...
    WScript.Echo ""
    WScript.Echo "Completing conversion..."
    WshShell.AppActivate "Autodesk Inventor"
    WScript.Sleep 500
    ...
    If partDoc.SubType = kSheetMetalSubType Then
        ...
        ' Step 8: Create flat pattern if not exists
    ...
    If Err.Number <> 0 Then
        ...
        Else
            ...
            If Not lengthProp Is Nothing Then
                ...
                WScript.Echo "Set PLATE LENGTH precision to 0 decimals"
            End If
        End If
    Else
    ...
    If Err.Number <> 0 Then
        ...
        Else
            ...
            If Not widthProp Is Nothing Then
                ...
                WScript.Echo "Set PLATE WIDTH precision to 0 decimals"
            End If
        End If
    Else
        ' Property exists - update with formula
