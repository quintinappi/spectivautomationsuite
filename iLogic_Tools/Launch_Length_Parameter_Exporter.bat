...
cls
echo =========================================================
echo LENGTH PARAMETER EXPORTER
echo =========================================================
echo.
echo This tool enables parameter export for NON-plate parts.
echo.
echo REQUIREMENTS:
echo - Inventor must be running
echo - An ASSEMBLY document (.iam) must be open (not a part file)
echo - Parts must contain Length parameters to export
echo.
echo PROCESS:
echo 1. Scans BOM for parts that DON'T contain "PL" or "S355JR"
echo 2. For each non-plate part:
echo    - Opens the part file
echo    - Finds the "Length" user parameter
echo    - Enables export for that parameter (checks the Export Param checkbox)
echo    - Saves the part
echo.
echo NOTE: This tool is useful for preparing parts for bulk export
...
echo.
echo Starting Length Parameter Exporter...
...
cscript //nologo "Length_Parameter_Exporter.vbs"
