' Sheet-Specific Parts List Creator - DETAILING WORKFLOW STEP 10: Create GA PARTS LIST
' DETAILING WORKFLOW - STEP 10: Create GA PARTS LIST
...
Sub CreateFilteredPartsList(sheet, visibleComponents)
    ...
    ' Find all sheets with assembly views
    Dim assemblySheetsDict
    Set assemblySheetsDict = CreateObject("Scripting.Dictionary")
            ...
            If Not refDesc Is Nothing Then
                ...
                If Not refDoc Is Nothing Then
                    If LCase(Right(refDoc.FullFileName, 4)) = ".iam" Then
                        If Not assemblySheetsDict.Exists(sh.Name) Then
                            assemblySheetsDict.Add sh.Name, v
    ...
    If assemblySheetsDict.Count = 0 Then
        WScript.Echo "ERROR: No assembly views found in drawing to create parts list from"
        Exit Sub
    End If

    ' If only one assembly sheet, use it automatically
    Dim assemblyView
    If assemblySheetsDict.Count = 1 Then
        Dim onlyKey
        onlyKey = assemblySheetsDict.Keys()(0)
        Set assemblyView = assemblySheetsDict.Item(onlyKey)
        WScript.Echo "Using only assembly found on sheet: " & onlyKey
    Else
        ' Multiple assembly sheets - ask user to choose
        WScript.Echo ""
        WScript.Echo "Multiple sheets with assemblies found:"
        WScript.Echo ""
        
        Dim i, sheetNames
        sheetNames = assemblySheetsDict.Keys()
        For i = 0 To assemblySheetsDict.Count - 1
            WScript.Echo (i + 1) & ". " & sheetNames(i)
        Next
        
        WScript.Echo ""
        WScript.StdOut.Write "Enter the number of the sheet to use as basis for parts list: "
        ...
        If selectedIndex < 0 Or selectedIndex >= assemblySheetsDict.Count Then
        ...
        Set assemblyView = assemblySheetsDict.Item(selectedSheet)
        WScript.Echo "Using assembly from sheet: " & selectedSheet
    End If

    If assemblyView Is Nothing Then
        WScript.Echo "ERROR: Failed to select assembly view"
        Exit Sub
    End If
    ...
    ' Renumber visible rows as 01, 02, etc.
    Dim itemNum
    itemNum = 1
    For Each row In partsList.PartsListRows
        If row.Visible Then
            row.Item = Right("0" & CStr(itemNum), 2)
            itemNum = itemNum + 1
        End If
    Next

    WScript.Echo "Renumbered visible items starting from 01"
