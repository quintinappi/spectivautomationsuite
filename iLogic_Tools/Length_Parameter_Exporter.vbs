' Length_Parameter_Exporter.vbs - DETAILING WORKFLOW STEP 5: Enable export for non-plate length properties
' DETAILING WORKFLOW - STEP 5: Enable export for non-plate length properties
' Length Parameter Exporter - Standalone VBScript
' Enables export for Length parameter on NON-plate parts
...
Sub Main()
    ...
    LogMessage "=== LENGTH PARAMETER EXPORTER STARTED ==="
    ...
    If nonPlateParts.Count = 0 Then
        LogMessage "No non-plate parts found in BOM"
    ...
    ' Step 2: Process each non-plate part
    LogMessage "STEP 2: Processing non-plate parts"
    Dim partPath
    For Each partPath In nonPlateParts
        LogMessage ""
        ...
        Else
            ...
            ' Enable Length parameter export
            If EnableLengthParameterExport(partDoc) Then
                LogMessage "Successfully enabled Length parameter export"
                ...
                On Error GoTo 0
            Else
                LogMessage "WARNING: Length parameter not found or already exported"
    ...
    LogMessage "Parts skipped (no Length param): " & skippedCount
    ...
    MsgBox "Processing complete!" & vbCrLf & vbCrLf & _
           ...
           "Skipped (no Length param): " & skippedCount & " parts" & vbCrLf & _
...
Function ScanAssemblyForNonPlateParts(asmDoc)
    ' Returns a collection of full file paths for parts that DON'T contain "PL" or "S355JR"
    Dim result
...
Function EnableLengthParameterExport(partDoc)
    ' Enables export for the "Length" user parameter
    ...
    ' Look for "Length" parameter
    For Each param In userParams
        LogMessage "Checking parameter: " & param.Name
        If UCase(param.Name) = "LENGTH" Then
            LogMessage "Found Length parameter"
            If Not param.ExportedToSheet Then
                LogMessage "Enabling export for Length parameter"
...
Sub SaveLog()
    ...
    m_LogPath = logFolder & "\Length_Parameter_Export_" & Replace(Replace(Replace(Now, "/", "-"), ":", "-"), " ", "_") & ".log"
