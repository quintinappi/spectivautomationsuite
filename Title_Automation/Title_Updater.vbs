' ===============================================================================
' BASE VIEW TITLE UPDATER - DETAILING WORKFLOW STEP 15: Update Titles
' ===============================================================================
' DETAILING WORKFLOW - STEP 15: Update titles to Pentalin style after detailing
'
...
Sub UpdateAllBaseViewTitles()
    ...
    ' Ask user whether to process all sheets or just current sheet
    Dim processAllSheets, userChoice
    processAllSheets = True  ' Default to all sheets
    
    userChoice = InputBox("Process titles for:" & vbCrLf & vbCrLf & _
                          "A = All sheets" & vbCrLf & _
                          "C = Current sheet only" & vbCrLf & vbCrLf & _
                          "Enter A or C:", _
                          "Sheet Selection", "A")
    
    If UCase(Trim(userChoice)) = "C" Then
        processAllSheets = False
        WScript.Echo "Processing: CURRENT SHEET ONLY"
    Else
        WScript.Echo "Processing: ALL SHEETS"
    End If
    WScript.Echo ""
    ...
    If processAllSheets Then
        ...
        Next
    Else
        ' Process current/active sheet only
        Dim currentSheet
        Set currentSheet = drawingDoc.ActiveSheet
        ...
        WScript.Echo "SHEET (Current): " & currentSheet.Name
        WScript.Echo "--------------------"
        
        totalUpdated = UpdateSheetBaseViewTitles(currentSheet, mainAssemblyDoc)
        WScript.Echo ""
    End If
...
Function UpdateNonBaseViewTitle(view)
    ...
    ' Projected views: Keep view description from first line, apply styling
    Dim newTitle
    newTitle = CreateProjectedViewTitle(view, scaleString)
    
    WScript.Echo "  PROJECTED VIEW: " & view.Name & " -> keeping description with new styling"
...
Function CreateProjectedViewTitle(view, scaleString)
    ' Projected view format:
    ' View Name (read dynamically from view.Name - NOT a formula)
    ' SCALE 1:X           (2.5mm = 0.25cm) - THIS IS A FORMULA
    
    On Error Resume Next
    
    ' Get current label text to extract the custom description
    Dim currentText, firstLine
    firstLine = view.Name  ' Default to view name
    
    If Not view.Label Is Nothing Then
        currentText = view.Label.Text
        If currentText <> "" Then
            ' Extract first line (everything before first newline)
            Dim pos
            pos = InStr(currentText, vbCrLf)
            If pos = 0 Then pos = InStr(currentText, vbLf)
            If pos = 0 Then pos = InStr(currentText, vbCr)
            
            If pos > 0 Then
                firstLine = Left(currentText, pos - 1)
            Else
                ' No newline found, use entire text
                firstLine = currentText
            End If
        End If
    End If
    
    ' Clean up first line - REMOVE any existing scale text
    ' Common patterns: "SCALE 1:20", "1:20", "SCALE 1 : 20"
    firstLine = Trim(firstLine)
    
    ' Remove "SCALE" and any scale values from the first line
    Dim scalePos
    scalePos = InStr(UCase(firstLine), "SCALE")
    If scalePos > 0 Then
        firstLine = Trim(Left(firstLine, scalePos - 1))
    End If
    
    ' Also check for patterns like "VIEW11 VIEW" (remove trailing view/scale words)
    ' Remove anything that looks like a scale ratio (e.g., "1:20", "1 : 20")
    Dim regexPattern
    regexPattern = "\s*\d+\s*[:\-]\s*\d+\s*$"  ' Matches " 1:20" or " 1 : 20" at end
    
    ' Simple string approach - remove ratio-like patterns from end
    Dim i, ch, foundRatio
    foundRatio = False
    For i = Len(firstLine) To 1 Step -1
        ch = Mid(firstLine, i, 1)
        ' If we hit a digit or colon/space, keep going back
        If IsNumeric(ch) Or ch = ":" Or ch = " " Or ch = "-" Then
            foundRatio = True
        Else
            ' Found a non-ratio character, stop here
            If foundRatio Then
                firstLine = Trim(Left(firstLine, i))
            End If
            Exit For
        End If
    Next
    
    ' Clean up any trailing "VIEW" text that might be left
    If UCase(Right(firstLine, 4)) = "VIEW" Then
        firstLine = Trim(Left(firstLine, Len(firstLine) - 4))
    End If
    
    ' NOTE: Only <DrawingViewScale/> works as a formula in Inventor.
    Dim title
    title = "<StyleOverride FontSize='0.35' Bold='True' Underline='True'>" & firstLine & "</StyleOverride>" & vbCrLf & _
            "<StyleOverride FontSize='0.25' Bold='False' Underline='False'>SCALE <DrawingViewScale/></StyleOverride>"
    
    CreateProjectedViewTitle = title
End Function
...
Function CreatePartTitle(partNumber, scaleString)
    ...
    ' FL50x3x3000         (2.5mm = 0.25cm)
    ' SCALE 1:5           (2.5mm = 0.25cm)

    Dim title
    title = "<StyleOverride FontSize='0.35' Bold='True' Underline='True'><Property Document='model' PropertySet='Design Tracking Properties' Property='Part Number' FormatID='{32853F0F-3444-11D1-9E93-0060B03C1CA6}' PropertyID='5'>PART NUMBER</Property></StyleOverride>" & vbCrLf & _
            "<StyleOverride FontSize='0.25' Bold='False' Underline='False'><Property Document='model' PropertySet='Design Tracking Properties' Property='Stock Number' FormatID='{32853F0F-3444-11D1-9E93-0060B03C1CA6}' PropertyID='55'>STOCK NUMBER</Property></StyleOverride>" & vbCrLf & _
            "<StyleOverride FontSize='0.25' Bold='False' Underline='False'>SCALE <DrawingViewScale/></StyleOverride>"
